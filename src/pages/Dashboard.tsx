      ...
      <div className="p-6 lg:p-8 max-w-7xl mx-auto">
        ...
        >
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold text-foreground">
              Good {getGreeting()}, {profile?.name?.split(' ')[0] || user?.email?.split('@')[0] || 'User'}
            </h1>
            <p className="text-muted-foreground mt-1">
              {getStatusMessage(todayCompleted, habits.length)}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <StatsCard
            ...
            value={`${streak} days`}
            subtitle={streak >= 7 ? "Consistent streak" : "Current sequence"}
            icon={Flame}
            color="accent"
            delay={0.1}
          />
...
function getStatusMessage(completed: number, total: number): string {
  if (total === 0) return "No habits tracked.";
  const ratio = completed / total;
  if (ratio === 1) return "All targets met.";
  if (ratio >= 0.7) return `${Math.round(ratio * 100)}% of targets met.`;
  if (ratio >= 0.3) return "Progress recorded.";
  if (completed > 0) return "Activity started.";
